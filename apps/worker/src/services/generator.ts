...
- ユーザーの入力は「素材・メモ書き」として扱い、**そのまま記事に転記しない**こと
- 箇条書き・メモ調・体言止め・「〜が以下」「〜したのが以下」などの横着な表現は、読者に向けた自然な文章に書き直すこと
  - 悪い例：「電源ONしたのが以下。」「実際に設置して電源ONした写真が以下です。」
  - 良い例：「実際にセットアップしてみると、ライトは想像以上に明るく、モニター周りがぱっと明るくなりました。」
- 記事の目的は**閲覧者に一次情報を分かりやすく届けること**。読者が読んで価値を感じられる、丁寧で読みやすい文章を心がけること
- 著者の実体験・感想をベースに、読者が「買おうか迷っている・行こうか考えている」状況で役立つ情報を自然な文体で表現すること
